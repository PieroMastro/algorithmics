...
from instructions import instructions_text, test1_text, test2_text, test3_text, sits_text
from logic import ruffier_index
...
class Pulse1Screen(Screen):
    def __init__(self, **kwargs):
        ...
        inst_text = Label(text=test1_text, halign='center', valign='top')
        ...
        self.button.on_press = self.next
        # 🟢 Inicialización de la clase Timer con el nuevo constructor
        self.timer = Timer(total=3)
...
class SitsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.timer = Timer(total=5)
        self.timer.bind(done=self.sec_finished)
        inst_text = Label(text=test2_text, halign='center', valign='top')
...
class Pulse2Screen(Screen):
    def __init__(self, **kwargs):
        ...
        inst_text = Label(text=test3_text, halign='center', valign='top', size_hint_y=0.2)
        self.status_label = Label(text='Pulsa "Iniciar P2" para iniciar la medición', size_hint_y=0.1)

        # 🟢 Inicialización de la clase Timer con el nuevo constructor (usamos 15s como valor inicial)
        self.timer = Timer(total=3)
        self.timer.bind(done=self.timer_finished)
...
class ResultScreen(Screen):
    ...
    def on_enter(self, *args):
        ...
        self.resultado.text = f'Registros:\nPulse 1: {pulse_1} - Pulse_2: {pulse_2} - Pulse_3: {pulse_3}'
