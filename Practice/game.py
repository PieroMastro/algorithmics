...
# Variables globales (constantes de ajustes)
SCREEN_WIDTH, SCREEN_HEIGHT = 800, 600
LEFT_BOUND = SCREEN_WIDTH / 40             # límites que el jugador no supera (el fondo empieza a moverse)
RIGHT_BOUND = SCREEN_WIDTH - 8 * LEFT_BOUND
SHIFT = 0
FPS = 60

x_start, y_start = 20, 10

img_file_background = 'cave.png'
...
# Empieza el juego
pygame.display.set_caption("PROJECT ARCADE")
window = pygame.display.set_mode([SCREEN_WIDTH, SCREEN_HEIGHT])

background = pygame.transform.scale(pygame.image.load(img_file_background).convert(), (SCREEN_WIDTH, SCREEN_HEIGHT))
...
pr = FinalSprite(img_file_princess, SCREEN_WIDTH + 500, SCREEN_HEIGHT - 150, 0)
all_sprites.add(pr)
...
while run:
    ...
    if not finished:
        ...
        local_SHIFT = SHIFT % SCREEN_WIDTH
        window.blit(background, (local_SHIFT, 0))
        if local_SHIFT != 0:
            window.blit(background, (local_SHIFT - SCREEN_WIDTH, 0))
        ...
        if robin not in all_sprites or robin.rect.top > SCREEN_HEIGHT:
            finished = True           
            window.fill(BLACK)
            # escribimos texto en la pantalla
